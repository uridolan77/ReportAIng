...
class TuningApiService {
  ...
  async updateBusinessTable(table: BusinessTableInfo): Promise<BusinessTableInfo> {
    return this.updateBusinessTableInfo(table.id, table);
  }
  ...
  async updateQueryPattern(pattern: QueryPattern): Promise<QueryPattern> {
    return this.updatePattern(pattern.id, pattern);
  }

  async getPromptLogs(page = 1, pageSize = 20): Promise<PromptLog[]> {
    return this.getPromptLogs({ page, pageSize });
  }

  async autoGenerateTableDescriptions(): Promise<AutoGeneratedTableContext[]> {
    return this.autoGenerateTableContexts();
  }

  async autoGenerateGlossary(): Promise<AutoGeneratedGlossaryTerm[]> {
    return this.autoGenerateGlossaryTerms();
  }
