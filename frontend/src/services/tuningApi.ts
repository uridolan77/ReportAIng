...
class TuningApiService {
  ...
  private adaptBackendResponse(backendResponse: any): AutoGenerationResponse {
    ...
    const adapted: AutoGenerationResponse = {
      ...
      totalTablesProcessed: backendResponse.totalTablesProcessed || (backendResponse.generatedTables?.length || 0),
      totalColumnsProcessed: backendResponse.totalColumnsProcessed || 0,
      totalTermsGenerated: backendResponse.totalTermsGenerated || (backendResponse.generatedTerms?.length || 0),
  ...
  // Save auto-generated content to database
  async saveGeneratedTableContext(context: AutoGeneratedTableContext): Promise<BusinessTableInfo> {
    const createRequest: CreateTableRequest = {
      tableName: context.tableName,
      schemaName: context.schemaName || 'dbo',
    ...
    return this.createBusinessTable(createRequest);
  }

  async saveGeneratedGlossaryTerm(term: AutoGeneratedGlossaryTerm): Promise<BusinessGlossaryTerm> {
    const createRequest: BusinessGlossaryTerm = {
    ...
    return this.createGlossaryTerm(createRequest);
  }

  // Bulk save operations
  async saveGeneratedContent(response: AutoGenerationResponse): Promise<{
    savedTableContexts: BusinessTableInfo[];
    savedGlossaryTerms: BusinessGlossaryTerm[];
    errors: string[];
  }> {
    const savedTableContexts: BusinessTableInfo[] = [];
    const savedGlossaryTerms: BusinessGlossaryTerm[] = [];
    const errors: string[] = [];

    // Save table contexts
    for (const context of response.GeneratedTableContexts) {
      try {
        const saved = await this.saveGeneratedTableContext(context);
        savedTableContexts.push(saved);
      } catch (error) {
        errors.push(`Failed to save table context for ${context.tableName}: ${error instanceof Error ? error.message : 'Unknown error'}`);
      }
    }

    // Save glossary terms
    for (const term of response.GeneratedGlossaryTerms) {
      try {
        const saved = await this.saveGeneratedGlossaryTerm(term);
        savedGlossaryTerms.push(saved);
      } catch (error) {
        errors.push(`Failed to save glossary term ${term.term}: ${error instanceof Error ? error.message : 'Unknown error'}`);
      }
    }

    return { savedTableContexts, savedGlossaryTerms, errors };
  }
