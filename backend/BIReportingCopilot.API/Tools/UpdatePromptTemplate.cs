...
/// <summary>
/// Utility script to update the sql_generation prompt template with improved aggregation rules
/// This tool helps maintain and update AI prompt templates for better SQL generation
/// </summary>
public class UpdatePromptTemplate
{
    public static async Task UpdateSqlGenerationTemplate(BICopilotContext context)
    {
        try
        {
            // Find the existing sql_generation template
            var template = await context.PromptTemplates
                .FirstOrDefaultAsync(t => t.Name == "sql_generation" && t.IsActive);

            if (template == null)
            {
                Console.WriteLine("sql_generation template not found!");
                return;
            }

            // Updated content with improved aggregation rules
            var updatedContent = @"You are an expert SQL developer specializing in business intelligence and gaming/casino data analysis.
...
ORDER BY column1 DESC";
            ...
            template.Description = "SQL generation template with improved aggregation rules and player data structure";
            template.UpdatedDate = DateTime.UtcNow;

            await context.SaveChangesAsync();

            Console.WriteLine($"Successfully updated prompt template '{template.Name}' to version {template.Version}");
            Console.WriteLine("The AI will now correctly handle 'Total' queries without adding unnecessary GROUP BY clauses.");
        }
        catch (Exception ex)
        {
            Console.WriteLine($"Error updating prompt template: {ex.Message}");
            throw;
        }
    }
}
