...
-- Migration Script for BI Reporting Copilot
-- =====================================================
...
BEGIN
    CREATE TABLE [dbo].[LLMCostTracking] (
        ...
        CONSTRAINT [FK_LLMCostTracking_Users] FOREIGN KEY ([UserId]) REFERENCES [dbo].[Users]([Id]) ON DELETE CASCADE
    )
...
BEGIN
    CREATE TABLE [dbo].[LLMResourceUsage] (
        ...
        CONSTRAINT [FK_LLMResourceUsage_Users] FOREIGN KEY ([UserId]) REFERENCES [dbo].[Users]([Id]) ON DELETE CASCADE,
...
BEGIN
    CREATE TABLE [dbo].[LLMResourceQuotas] (
        ...
        CONSTRAINT [FK_LLMResourceQuotas_Users] FOREIGN KEY ([UserId]) REFERENCES [dbo].[Users]([Id]) ON DELETE CASCADE,
...
BEGIN
    CREATE TABLE [dbo].[LLMCostPredictions] (
        ...
        CONSTRAINT [FK_LLMCostPredictions_Users] FOREIGN KEY ([UserId]) REFERENCES [dbo].[Users]([Id]) ON DELETE CASCADE,
...
-- =====================================================
-- 11. Insert Default Resource Quotas for Admin User
-- =====================================================
DECLARE @AdminUserId NVARCHAR(450)
SELECT @AdminUserId = Id FROM [dbo].[Users] WHERE Email = 'admin@bireportingcopilot.com'

IF @AdminUserId IS NOT NULL AND NOT EXISTS (SELECT 1 FROM [dbo].[LLMResourceQuotas] WHERE [UserId] = @AdminUserId)
BEGIN
    INSERT INTO [dbo].[LLMResourceQuotas] ([UserId], [ResourceType], [MaxQuantity], [PeriodSeconds], [ResetDate])
    VALUES
        (@AdminUserId, 'ai-requests', 10000, 86400, DATEADD(day, 1, GETUTCDATE())),
        (@AdminUserId, 'database-queries', 50000, 86400, DATEADD(day, 1, GETUTCDATE())),
        (@AdminUserId, 'cache-operations', 100000, 86400, DATEADD(day, 1, GETUTCDATE())),
        (@AdminUserId, 'report-generations', 1000, 86400, DATEADD(day, 1, GETUTCDATE()))

    PRINT 'Inserted default LLM resource quotas for admin user'
END
ELSE
    PRINT 'Admin user not found or LLM resource quotas already exist'
GO

-- =====================================================
-- 12. Create Views for Cost Analytics
-- =====================================================
...
PRINT '============================================='
PRINT 'Cost Optimization Tables Migration Complete'
PRINT '============================================='
