  ...
  ""businessRules"": ""...""
    ...
    private async Task<AutoGeneratedGlossaryTerm> GenerateGlossaryTermAsync(string term, SchemaMetadata schema, string? sourceTable = null, string? sourceColumn = null)
    {
        try
        {
            ...
            // Skip overly complex terms that cause AI parsing issues
            if (term.Length > 30 || term.Split(new char[] { '_', ' ' }, StringSplitOptions.RemoveEmptyEntries).Length > 5)
            {
                _logger.LogDebug("Term too complex for AI generation, using fallback: {Term}", term);
                return GenerateFallbackGlossaryTerm(term, sourceTable, sourceColumn);
            }
        ...
        {
            _logger.LogError(ex, "Error generating AI glossary term for: {Term}. Using fallback.", term);
            return GenerateFallbackGlossaryTerm(term, sourceTable, sourceColumn);
        }
    }

    private AutoGeneratedGlossaryTerm GenerateFallbackGlossaryTerm(string term, string? sourceTable = null, string? sourceColumn = null)
    {
        // Generate a reasonable fallback term without AI
        var cleanTerm = term.Replace("_", " ").Replace("tbl", "").Trim();
        var category = DetermineTermCategory(term);

        return new AutoGeneratedGlossaryTerm
        {
            Term = term,
            Definition = $"Business term: {cleanTerm}",
            BusinessContext = $"Context for {cleanTerm} in business operations",
    ...
    private string DetermineTermCategory(string term)
    {
        ...
        if (lowerTerm.Contains("id") || lowerTerm.Contains("key")) return "Identifier";
        if (lowerTerm.Contains("date") || lowerTerm.Contains("time")) return "Temporal";
        if (lowerTerm.Contains("amount") || lowerTerm.Contains("balance") || lowerTerm.Contains("money")) return "Financial";
        if (lowerTerm.Contains("name") || lowerTerm.Contains("title") || lowerTerm.Contains("description")) return "Descriptive";
        if (lowerTerm.Contains("status") || lowerTerm.Contains("state") || lowerTerm.Contains("flag")) return "Status";
        return "Business";
    }
    ...
    private async Task<AutoGenerationResponse> GenerateBusinessContextInternalAsync(AutoGenerationRequest request, string userId, CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            try
            {
                ...
                if (!isHealthy)
                {
                    _logger.LogWarning("⚠️ AI Service is not healthy - will use fallback responses");
                }
            }
            catch (Exception ex)
            {
            ...
            if (request.GenerateTableContexts)
            {
                try
                {
                    if (request.SpecificTables?.Any() == true)
                    {
                        ...
                        response.GeneratedTableContexts = await GenerateSpecificTableContextsAsync(request.SpecificTables, request.SpecificFields);
                    }
                    else
                    {
                ...
                {
                    _logger.LogError(ex, "❌ Error generating table contexts, using fallback");

                    // Generate fallback contexts if AI fails
                    response.GeneratedTableContexts = await GenerateFallbackTableContextsAsync(request.SpecificTables);
                }
            }
            ...
            if (request.GenerateGlossaryTerms)
            {
                ...
                {
                    _logger.LogError(ex, "❌ Error generating glossary terms, using fallback");
                    response.GeneratedGlossaryTerms = await GenerateFallbackGlossaryTermsAsync(request.SpecificTables);
                }
            }
            else
            {
    ...
    /// <summary>
    /// Generate contexts for specific selected tables and fields
    /// </summary>
...
Focus specifically on the selected fields: {string.Join(", ", selectedFieldNames)}";
    ...
    private AutoGeneratedTableContext ParseTableContextResponse(string aiResponse, TableMetadata tableMetadata, List<string> selectedFieldNames)
    {
        try
        {
            ...
            {
                _logger.LogWarning("⚠️ AI response is empty for table {TableName}, using fallback", tableMetadata.Name);
                return CreateFallbackTableContext(tableMetadata, selectedFieldNames);
            }
            ...
            {
                _logger.LogWarning("⚠️ AI response doesn't appear to be JSON for table {TableName}: '{Response}', using fallback",
                    tableMetadata.Name, cleanResponse.Length > 100 ? cleanResponse.Substring(0, 100) + "..." : cleanResponse);
                return CreateFallbackTableContext(tableMetadata, selectedFieldNames);
            }
    ...
    /// <summary>
    /// Create fallback table context when AI parsing fails
    /// </summary>
    private AutoGeneratedTableContext CreateFallbackTableContext(TableMetadata tableMetadata, List<string> selectedFieldNames)
    {
        _logger.LogInformation("🔄 Creating fallback context for table {TableName} with {FieldCount} selected fields",
            tableMetadata.Name, selectedFieldNames.Count);

        // Create meaningful business context based on table and field names
        var tableName = tableMetadata.Name.Replace("tbl_", "").Replace("_", " ");
        var businessPurpose = GenerateBusinessPurpose(tableMetadata.Name, selectedFieldNames);
        var businessContext = GenerateBusinessContext(tableMetadata.Name, selectedFieldNames);
        ...
        return new AutoGeneratedTableContext
        {
            ...
            BusinessPurpose = businessPurpose,
            BusinessContext = businessContext,
            PrimaryUseCase = $"Managing and querying {tableName} data for business operations",
            ConfidenceScore = 0.6, // Higher confidence for fallback since it's based on actual schema
            KeyBusinessMetrics = GenerateKeyMetrics(selectedFieldNames),
            CommonQueryPatterns = GenerateCommonPatterns(tableMetadata.Name, selectedFieldNames),
            BusinessRules = "Standard business rules apply for data integrity and access control",
            GeneratedAt = DateTime.UtcNow,
            IsAutoGenerated = true,
            Columns = selectedFieldNames.Select(fieldName => CreateFallbackColumnContext(fieldName, tableMetadata)).ToList(),
            RelatedTables = new List<string>()
        };
    }

    /// <summary>
    /// Generate business purpose based on table name and selected fields
    /// </summary>
    private string GenerateBusinessPurpose(string tableName, List<string> selectedFields)
    {
        var cleanTableName = tableName.Replace("tbl_", "").Replace("_", " ");

        // Analyze field names to understand the business domain
        var hasId = selectedFields.Any(f => f.ToLower().Contains("id"));
        var hasName = selectedFields.Any(f => f.ToLower().Contains("name"));
        var hasDate = selectedFields.Any(f => f.ToLower().Contains("date") || f.ToLower().Contains("time"));
        var hasAmount = selectedFields.Any(f => f.ToLower().Contains("amount") || f.ToLower().Contains("price") || f.ToLower().Contains("cost"));

        if (tableName.ToLower().Contains("country") || tableName.ToLower().Contains("countries"))
            return "Stores geographical and administrative information about countries including codes, currencies, and localization settings";
        if (tableName.ToLower().Contains("currency") || tableName.ToLower().Contains("currencies"))
            return "Manages currency information including exchange rates, symbols, and currency codes for multi-currency operations";
        if (tableName.ToLower().Contains("daily") && tableName.ToLower().Contains("action"))
            return "Tracks daily business activities and metrics including transactions, user actions, and operational data";
        if (tableName.ToLower().Contains("user") || tableName.ToLower().Contains("player"))
            return "Manages user account information, preferences, and profile data";
        if (tableName.ToLower().Contains("transaction") || tableName.ToLower().Contains("payment"))
            return "Records financial transactions and payment processing information";

        // Generic purpose based on field analysis
        if (hasAmount && hasDate)
            return $"Tracks financial and temporal data for {cleanTableName} with monetary values and time-based information";
        if (hasId && hasName)
            return $"Master data table for {cleanTableName} containing identification and descriptive information";
        if (hasDate)
            return $"Time-series data for {cleanTableName} tracking changes and events over time";

        return $"Business data storage for {cleanTableName} containing operational information";
    }

    /// <summary>
    /// Generate business context based on table name and selected fields
    /// </summary>
    private string GenerateBusinessContext(string tableName, List<string> selectedFields)
    {
        var cleanTableName = tableName.Replace("tbl_", "").Replace("_", " ");
        var fieldCount = selectedFields.Count;

        var context = $"This table contains {cleanTableName} information with {fieldCount} selected fields. ";

        // Add context based on field patterns
        var keyFields = selectedFields.Where(f =>
            f.ToLower().Contains("id") ||
            f.ToLower().Contains("name") ||
            f.ToLower().Contains("code")).ToList();

        if (keyFields.Any())
        {
            context += $"Key identifying fields include: {string.Join(", ", keyFields.Take(3))}. ";
        }

        var dateFields = selectedFields.Where(f =>
            f.ToLower().Contains("date") ||
            f.ToLower().Contains("time") ||
            f.ToLower().Contains("updated")).ToList();

        if (dateFields.Any())
        {
            context += $"Temporal tracking is provided through: {string.Join(", ", dateFields.Take(2))}. ";
        }

        context += "This data is used for business operations, reporting, and analytics.";

        return context;
    }

    /// <summary>
    /// Generate key business metrics based on selected fields
    /// </summary>
    private List<string> GenerateKeyMetrics(List<string> selectedFields)
    {
        var metrics = new List<string> { "Record Count", "Data Quality Score" };

        if (selectedFields.Any(f => f.ToLower().Contains("amount") || f.ToLower().Contains("price")))
            metrics.Add("Total Amount");
        if (selectedFields.Any(f => f.ToLower().Contains("count") || f.ToLower().Contains("quantity")))
            metrics.Add("Total Count");
        if (selectedFields.Any(f => f.ToLower().Contains("date") || f.ToLower().Contains("time")))
            metrics.Add("Data Freshness");
        if (selectedFields.Any(f => f.ToLower().Contains("active") || f.ToLower().Contains("status")))
            metrics.Add("Active Records");

        return metrics.Take(5).ToList();
    }

    /// <summary>
    /// Generate common query patterns based on table and fields
    /// </summary>
    private List<string> GenerateCommonPatterns(string tableName, List<string> selectedFields)
    {
        ...
        var idField = selectedFields.FirstOrDefault(f => f.ToLower().Contains("id"));
        if (!string.IsNullOrEmpty(idField))
            patterns.Add($"SELECT * FROM {tableName} WHERE {idField} = ?");

        var nameField = selectedFields.FirstOrDefault(f => f.ToLower().Contains("name"));
        if (!string.IsNullOrEmpty(nameField))
            patterns.Add($"SELECT * FROM {tableName} WHERE {nameField} LIKE '%?%'");

        var dateField = selectedFields.FirstOrDefault(f => f.ToLower().Contains("date") || f.ToLower().Contains("time"));
        if (!string.IsNullOrEmpty(dateField))
            patterns.Add($"SELECT * FROM {tableName} WHERE {dateField} >= ?");

        return patterns.Take(5).ToList();
    }

    /// <summary>
    /// Create fallback column context
    /// </summary>
    private AutoGeneratedColumnContext CreateFallbackColumnContext(string fieldName, TableMetadata tableMetadata)
    {
        var column = tableMetadata.Columns.FirstOrDefault(c => c.Name.Equals(fieldName, StringComparison.OrdinalIgnoreCase));
        var businessName = fieldName.Replace("_", " ").Replace("ID", "Identifier");
        var businessDescription = GenerateColumnDescription(fieldName);
        ...
        return new AutoGeneratedColumnContext
        {
            ...
            BusinessDataType = MapToBusinessDataType(column?.DataType ?? "varchar"),
            ConfidenceScore = 0.7, // Higher confidence for schema-based fallback
            IsAutoGenerated = true,
    ...
    /// <summary>
    /// Generate meaningful column description based on field name
    /// </summary>
    private string GenerateColumnDescription(string fieldName)
    {
        var lower = fieldName.ToLower();

        if (lower.Contains("id") && lower.Contains("country"))
            return "Unique identifier for country records";
        if (lower.Contains("id") && lower.Contains("currency"))
            return "Unique identifier for currency records";
        if (lower.Contains("name") && lower.Contains("country"))
            return "Full name of the country";
        if (lower.Contains("name") && lower.Contains("currency"))
            return "Full name of the currency";
        if (lower.Contains("code") && lower.Contains("country"))
            return "Standard country code (ISO format)";
        if (lower.Contains("code") && lower.Contains("currency"))
            return "Standard currency code (ISO 4217 format)";
        if (lower.Contains("rate"))
            return "Exchange rate value for currency conversion";
        if (lower.Contains("symbol"))
            return "Currency symbol for display purposes";
        if (lower.Contains("phone"))
            return "International phone code for the country";
        if (lower.Contains("active") || lower.Contains("isactive"))
            return "Indicates whether the record is currently active";
        if (lower.Contains("updated") && lower.Contains("date"))
        ...
        if (lower.EndsWith("id"))
            return $"Unique identifier for {fieldName.Replace("ID", "").Replace("_", " ").ToLower()}";

        return $"Business field containing {fieldName.Replace("_", " ").ToLower()} information";
    }
...
- Use double quotes for all JSON keys and string values";
    ...
    /// <summary>
    /// Generate fallback table contexts when AI service is not available
    /// </summary>
    private async Task<List<AutoGeneratedTableContext>> GenerateFallbackTableContextsAsync(List<string>? specificTables)
    {
        try
        {
            _logger.LogInformation("🔄 Generating fallback table contexts");

            var schema = await _schemaService.GetSchemaMetadataAsync();
            var tablesToProcess = specificTables?.Any() == true
                ? schema.Tables.Where(t => specificTables.Contains(t.Name, StringComparer.OrdinalIgnoreCase)).ToList()
                : schema.Tables.Take(3).ToList(); // Limit to first 3 tables for fallback

            var fallbackContexts = new List<AutoGeneratedTableContext>();

            foreach (var table in tablesToProcess)
            {
                var context = new AutoGeneratedTableContext
                {
                    TableName = table.Name,
                    SchemaName = table.Schema,
                    BusinessPurpose = $"Business data storage for {table.Name.Replace("_", " ").Replace("tbl", "").Trim()}",
                    BusinessContext = $"This table contains {table.Name.Replace("_", " ").ToLower()} information used for business operations and reporting.",
                    PrimaryUseCase = $"Primary use case for managing and querying {table.Name.Replace("_", " ").ToLower()} data",
                    ConfidenceScore = 0.5, // Lower confidence for fallback
                    KeyBusinessMetrics = new List<string> { "Record Count", "Data Quality" },
                    CommonQueryPatterns = new List<string> { $"SELECT * FROM {table.Name}", $"SELECT COUNT(*) FROM {table.Name}" },
                    BusinessRules = "Standard business rules apply for data integrity and access control",
                    GeneratedAt = DateTime.UtcNow,
                    IsAutoGenerated = true,
                    Columns = table.Columns.Take(10).Select(col => new AutoGeneratedColumnContext
                    {
                        ColumnName = col.Name,
                        BusinessName = col.Name.Replace("_", " "),
                        BusinessDescription = $"Business field for {col.Name.Replace("_", " ").ToLower()}",
                        DataType = col.DataType,
                        BusinessDataType = MapToBusinessDataType(col.DataType),
                        ConfidenceScore = 0.5,
                        IsAutoGenerated = true,
            ...
            _logger.LogInformation("✅ Generated {Count} fallback table contexts", fallbackContexts.Count);
            return fallbackContexts;
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "❌ Error generating fallback table contexts");
            return new List<AutoGeneratedTableContext>();
        }
    }

    /// <summary>
    /// Generate fallback glossary terms when AI service is not available
    /// </summary>
    private async Task<List<AutoGeneratedGlossaryTerm>> GenerateFallbackGlossaryTermsAsync(List<string>? specificTables)
    {
        try
        {
            _logger.LogInformation("🔄 Generating fallback glossary terms");
            ...
            var fallbackTerms = new List<AutoGeneratedGlossaryTerm>();

            foreach (var table in tablesToProcess)
            {
                // Add table-level term
                var tableTerm = new AutoGeneratedGlossaryTerm
                {
                    Term = table.Name.Replace("_", " ").Replace("tbl", "").Trim(),
                    Definition = $"Database table containing {table.Name.Replace("_", " ").ToLower()} information",
                    BusinessContext = $"Used for storing and managing {table.Name.Replace("_", " ").ToLower()} data in the system",
                    Category = "Database Tables",
                    Synonyms = new List<string> { table.Name },
                    RelatedTerms = new List<string>(),
                    ExampleValues = new List<string> { $"Query {table.Name} for reporting", $"Update {table.Name} records" },
                    ConfidenceScore = 0.5,
                    SourceTables = new List<string> { table.Name },
                    SourceColumns = new List<string>(),
                    GeneratedAt = DateTime.UtcNow,
                    IsAutoGenerated = true
                };
                fallbackTerms.Add(tableTerm);

                // Add key column terms
                foreach (var column in table.Columns.Where(c => c.IsPrimaryKey || c.IsForeignKey).Take(3))
                {
                    var columnTerm = new AutoGeneratedGlossaryTerm
                    {
                        Term = column.Name.Replace("_", " "),
                        Definition = $"Database column representing {column.Name.Replace("_", " ").ToLower()}",
                        BusinessContext = $"Key field in {table.Name} table used for {(column.IsPrimaryKey ? "unique identification" : "referential relationships")}",
                        Category = "Database Columns",
                        Synonyms = new List<string> { column.Name },
                        RelatedTerms = new List<string> { tableTerm.Term },
                        ExampleValues = new List<string> { $"Filter by {column.Name}", $"Join on {column.Name}" },
                        ConfidenceScore = 0.5,
                        SourceTables = new List<string> { table.Name },
                        SourceColumns = new List<string> { column.Name },
                        GeneratedAt = DateTime.UtcNow,
                        IsAutoGenerated = true
                    };
                    fallbackTerms.Add(columnTerm);
                }
            }

            _logger.LogInformation("✅ Generated {Count} fallback glossary terms", fallbackTerms.Count);
            return fallbackTerms;
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "❌ Error generating fallback glossary terms");
            return new List<AutoGeneratedGlossaryTerm>();
        }
    }
