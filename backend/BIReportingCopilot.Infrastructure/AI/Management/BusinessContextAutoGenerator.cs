  ...
  ""businessRules"": ""...""
    ...
    private AutoGeneratedGlossaryTerm ParseGlossaryTermResponse(string aiResponse, string term, string? sourceTable = null, string? sourceColumn = null)
    {
        try
        {
            ...
            _logger.LogInformation("🤖 Raw AI response for glossary term {Term}: '{Response}' (Length: {Length})",
                term, aiResponse, aiResponse?.Length ?? 0);

            // Check if response is empty or null
            if (string.IsNullOrWhiteSpace(aiResponse))
            {
                _logger.LogError("❌ AI response is empty for term {Term}", term);
                throw new InvalidOperationException($"AI response is empty for term {term}");
            }

            // Log first 200 characters for debugging
            var preview = aiResponse.Length > 200 ? aiResponse.Substring(0, 200) + "..." : aiResponse;
            _logger.LogInformation("🔍 AI response preview for {Term}: '{Preview}'", term, preview);

            // Clean up the response to extract JSON
            var jsonMatch = Regex.Match(aiResponse, @"\{.*\}", RegexOptions.Singleline);
            if (!jsonMatch.Success)
            {
                _logger.LogError("❌ No JSON found in AI response for term {Term}. Full response: '{Response}'", term, aiResponse);
                throw new InvalidOperationException($"AI response does not contain valid JSON for term {term}: '{aiResponse.Substring(0, Math.Min(100, aiResponse.Length))}'");
            }
    ...
    private async Task<AutoGeneratedGlossaryTerm> GenerateGlossaryTermAsync(string term, SchemaMetadata schema, string? sourceTable = null, string? sourceColumn = null)
    {
        try
        {
            ...
            {
                _logger.LogDebug("Term too complex for AI generation, skipping: {Term}", term);
                throw new InvalidOperationException($"Term too complex for AI generation: {term}");
            }

            // Build AI prompt for glossary term analysis
            _logger.LogInformation("🔨 Building prompt for glossary term: {Term}", term);
            var prompt = BuildGlossaryTermPrompt(term, schema, sourceTable, sourceColumn);
            _logger.LogInformation("📝 Built prompt for {Term}, length: {Length}", term, prompt.Length);

            // Get AI analysis using the correct method for business context generation
            _logger.LogInformation("🤖 Calling GenerateStructuredResponseAsync for term: {Term}", term);
            var aiResponse = await GenerateStructuredResponseAsync(prompt);
            _logger.LogInformation("📨 Received AI response for {Term}, length: {Length}", term, aiResponse?.Length ?? 0);

            // Parse AI response
            _logger.LogInformation("🔍 Parsing AI response for term: {Term}", term);
...
- Use double quotes for all JSON keys and string values";
            ...
            _logger.LogInformation("🤖 Calling AI service GenerateInsightAsync...");
