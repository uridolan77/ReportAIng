    ...
    public async Task<AutoGenerationResponse> AutoGenerateBusinessContextAsync(AutoGenerationRequest request, string userId)
    {
        ...
        try
        {
            ...
            if (request.GenerateGlossaryTerms)
            {
                ...
                await SendProgressUpdate(userId, 50, "Analyzing business terminology...", "Glossary Generation");

                List<AutoGeneratedGlossaryTerm> glossaryTerms;
                if (request.SpecificTables?.Any() == true)
                {
                    // Generate glossary terms for specific tables only
                    await SendProgressUpdate(userId, 55, $"Extracting terms from {request.SpecificTables.Count} selected tables...", "Glossary Generation");
                    ...
                    {
                        await SendProgressUpdate(userId, 55, message, stage, null, currentItem);
                    });

                    glossaryTerms = await _autoGenerator.GenerateGlossaryTermsAsync(request.SpecificTables!, glossaryProgressCallback);
                }
                else
                {
                    // Generate glossary terms for all tables
                    await SendProgressUpdate(userId, 55, "Extracting terms from all tables...", "Glossary Generation");
                    glossaryTerms = await _autoGenerator.GenerateGlossaryTermsAsync();
                }

                await SendProgressUpdate(userId, 75, $"Generated {glossaryTerms.Count} glossary terms", "Glossary Generation");
            ...
            if (request.AnalyzeRelationships)
            {
                ...
                await SendProgressUpdate(userId, 80, "Analyzing table relationships...", "Relationship Analysis");
                response.RelationshipAnalysis = await _autoGenerator.AnalyzeTableRelationshipsAsync();
                await SendProgressUpdate(userId, 90, "Completed relationship analysis", "Relationship Analysis");
            }
